...
class AuxiliaryTrainingHelper(TrainingHelper):
  """A helper for use during training.  Only reads inputs.
  ...
  def __init__(self, inputs, sequence_length, time_major=False,
    ...
    Args:
      ...
      auxiliary_inputs: An optional (structure of) auxiliary input tensors with
        a shape that matches `inputs` in batch_size dimension.
      name: Name scope for any created operations.

    Raises:
      ValueError: if `sampling_probability` is not a scalar or vector.
    """
    with ops.name_scope(name, "AuxiliaryTrainingHelper",
                        [inputs, auxiliary_inputs]):
      self.auxiliary_inputs_ = auxiliary_inputs
      super(AuxiliaryTrainingHelper, self).__init__(
      	inputs=inputs,
      	sequence_length=sequence_length,
      	time_major=time_major,
      	name=name)
      self._zero_inputs = array_ops.concat(
      	(self._zero_inputs, array_ops.zeros_like(self.auxiliary_inputs_)), axis = 1)
  ...
  def initialize(self, name=None):
    with ops.name_scope(name, "AuxiliaryTrainingHelperInitialize"):
      finished = math_ops.equal(0, self._sequence_length)
      all_finished = math_ops.reduce_all(finished)
      next_inputs = control_flow_ops.cond(
          all_finished, lambda: self._zero_inputs,
          lambda: nest.map_structure(lambda inp:
            array_ops.concat((inp.read(0), self.auxiliary_inputs_), axis=-1),
            self._input_tas))
          #lambda: nest.map_structure(lambda inp:
          #  inp.read(0), self._input_tas))
      return (finished, next_inputs)

  def next_inputs(self, time, outputs, state, name=None, **unused_kwargs):
    """next_inputs_fn for AuxiliaryTrainingHelper."""
    with ops.name_scope(name, "AuxiliaryTrainingHelperNextInputs",
                        [time, outputs, state]):
      next_time = time + 1
      finished = (next_time >= self._sequence_length)
      all_finished = math_ops.reduce_all(finished)
      def read_from_ta(inp):
        #return inp.read(next_time)
        return array_ops.concat((inp.read(next_time), self.auxiliary_inputs_),
          axis=-1)
      next_inputs = control_flow_ops.cond(
          all_finished, lambda: self._zero_inputs,
          lambda: nest.map_structure(read_from_ta, self._input_tas))
