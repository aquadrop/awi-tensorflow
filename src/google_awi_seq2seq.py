...
class AttentionSortModel:
    ...
    def _create_placeholder(self):
        ...
        with tf.variable_scope("decoder") as scope:
            ...
            self.decoder_inputs_length = tf.placeholder(tf.int32, shape=[None, 1], name="decoder_inputs_length")
    ...
    def _inference(self):
        ...
        #     # self.decoder_state_update_op = self.get_state_update_op(self.decoder_state, decoder_state)
            ...
            self.encoder_state_update_op = self.get_state_update_op(self.encoder_initial_bw_state, decoder_state)
...
def train():
    ...
    with tf.Session() as sess:
        ...
        for stei, stdi, stl in gen:
            stei_length = np.full(shape=AttentionSortModel.batch_size,
                                  fill_value=AttentionSortModel.ENCODER_NUM_STEPS, dtype=np.int32)
            # print(sess.run(model.loss, feed_dict={model.encoder_inputs.name: stei,\
            #                                model.encoder_inputs_length: stei_length,\
