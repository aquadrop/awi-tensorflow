...
def plus_op_data():
    while True:
        ...
        yield np.asarray(encoder_inputs, dtype=np.int32).reshape(batch_size, ENCODER_SEQ_LENGTH), np.asarray(
            decoder_inputs, dtype=np.int32).reshape((batch_size, DECODER_SEQ_LENGTH)), np.asarray(labels)
